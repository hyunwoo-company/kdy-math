...
10. **`.env` 파일 추가 금지** — 이 프로젝트는 환경변수를 쓰지 않는다. 비밀값이 필요한 기능이면 설계가 잘못된 것이다.
...
- ⚠️ **현재 `git push` 만으로는 사이트가 갱신되지 않는다.** GitHub 자동 배포가 아직 연결되지 않아, 실제 배포는 `npx vercel@latest --prod --yes --scope jenu8628s-projects` 로 이뤄진다. 연결 절차와 배포 방법은 [DEPLOY.md](DEPLOY.md) 4절에 있다. 사용자에게 "push했으니 배포됐다"고 말하지 마라.
