  ...
  return (
    ...
    >
      ...
      <div
        id="mobile-menu"
        hidden={!open}
        // 배경은 불투명이어야 한다. 반투명 + blur 는 backdrop-filter 미지원
        // 브라우저에서 본문이 비쳐 글자가 겹친다 (아래 "모바일 오버레이 규칙" 참고)
        className="fixed inset-x-0 bottom-0 top-12 z-40 bg-bg md:hidden"
      >
...
- **배경은 불투명(`bg-bg`)이어야 한다.** 반투명 + `backdrop-blur` 조합을 쓰지 마라.
  `backdrop-filter` 를 지원하지 않는 인앱 브라우저(카카오톡 등)에서는 흐림 효과 없이
  반투명만 남아 **뒤 본문의 큰 제목이 메뉴 글자와 겹쳐 읽을 수 없게 된다.**
  실제로 이 사이트에서 발생한 문제다(2026-08-21 수정). 글래스는 **고정 헤더 바 전용**이며,
  그 헤더도 `@utility glass-nav` 를 거쳐야 한다 — 이 유틸에만 `@supports not` 불투명
  폴백이 들어 있다. 헤더에 `bg-[rgba(...)] + backdrop-blur-*` 를 인라인으로 쓰면 폴백이 없다.
- 메뉴가 열린 동안에는 **헤더도 불투명**으로 바꾼다. 헤더만 반투명이면 경계가 어긋나 보인다.
...
- ~~헤더에 CTA 버튼 배치~~ → **이 사이트는 예외다.** Apple 은 헤더에 CTA 를 두지 않지만,
  이 사이트는 **명함 QR 로 들어와 상담으로 이어지는 것이 유일한 목적**이라 전환 경로를
  짧게 유지한다(2026-08-21 결정). 다만 범위를 좁게 지킨다.
  - **모바일 헤더에만** 둔다. 데스크톱은 상단 탭에 "상담 문의" 가 이미 보이므로 중복이다.
  - 위치는 햄버거 버튼 **왼쪽**, 높이 `36px`(`h-9`) · `px-4` · `14px`. 헤더가 48px 고정이라
    본문 pill(44px)을 그대로 쓰면 꽉 찬다. 구현은 `components/Nav.tsx` 의 `headerCtaClass`.
  - 문구·주소는 `content/site.ts` 의 `headerCta` 에서 가져온다.
  - **액센트는 여전히 1개다.** 이 버튼이 헤더의 유일한 액센트이며, 탭 링크에는 절대 쓰지 않는다.
- 오버레이 안에 CTA 버튼 배치 (링크만 — 이건 예외 없이 지킨다)
